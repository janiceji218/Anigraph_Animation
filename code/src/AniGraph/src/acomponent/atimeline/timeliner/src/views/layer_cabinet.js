...
const SHOWSAVELOADBUTTONS=false;
const UNDOREDO = false;
...
function LayerCabinet(data, dispatcher) {
	...
	range.type = "range";
	range.value = 0;
	range.min = LayoutConstants.TIMESCALE_RANGE_MIN;
	range.max = LayoutConstants.TIMESCALE_RANGE_MAX;
	range.step = (range.max-range.min)/200;
	// range.step = 0.125;
	...
	// ABENOTE TODO: You can uncomment this to have it add the save load operation buttons
	if(SHOWSAVELOADBUTTONS) {
		addLoadSaveToOperationsDiv();
	}

	function addLoadSaveToOperationsDiv(){
		// open _alt
