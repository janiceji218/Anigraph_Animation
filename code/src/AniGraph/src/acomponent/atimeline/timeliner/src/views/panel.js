...
function TimelinePanel(data, dispatcher) {
	...
	function Diamond(layer, frame, y) {
		var x, y2;
	...
	function drawLayerContents() {
		...
		for (i = 0; i < il; i++) {
			...
			for (j = 0; j < values.length; j++) {
				// Dimonds
				frame = values[j];
				renderItems.push(new Diamond(layer, frame, y));
			}
		}
	...
	track_canvas.addEventListener('dblclick', function(e) {
		...
		if(track<0 || track>(layers.length-1)){
			return;
		}
	...
	handleDrag(track_canvas, function down(e) {
		...
		if (mouseDownThenMove) {
			...
			dispatcher.fire('keyframe.move', mousedownItem, x_to_time(e.offsetx));
		}
		else {
			dispatcher.fire('time.update', x_to_time(e.offsetx));
		}
